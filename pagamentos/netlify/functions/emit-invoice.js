...
async function emitirFaturaVendus(dadosCliente, dadosProduto, dadosPagamento) {
  ...
  // Fallback para produtos não mapeados - usar nome do produto ou genérico
  const produtoVendus = PRODUTOS_VENDUS[dadosProduto.id] || {
    nome: dadosProduto.id?.includes('DONATIVO') ? 'Donativo BE WATER' : (dadosProduto.nome || 'Consumível Gym'),
    iva: 0,
